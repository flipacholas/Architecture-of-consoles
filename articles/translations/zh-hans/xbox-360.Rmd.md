...
此外，VMX128增加了新的指令，可以计算由多达三个32位浮点数组成的两个向量的**点积**；以及其他处理**Direct3D数据压缩格式**的指令[@cpu-brown]。值得注意的是，DirectX是这个控制台编程的唯一API。 最后，由于Xenon的对称设计及其多线程模型（双发射），VMX128的寄存器文件被复制，因此每个核心有**256个128位寄存器**！
...
这意味着该系统包含一些核心组件，负责处理控制台的低级功能，如硬件访问、安全防护和资源管理。 除此之外，还包括各种应用程序（例如启动动画、名为‘Dashboard’的交互式用户界面以及游戏本身）和用户数据（如游戏存档、用户配置文件和网络设置）。
...
- **竞争时代**（2008-2011）：随着PlayStation 3和Wii占据商店空间，微软发现自己需要积极争夺消费者的注意力。 首先，一个新的系统更新称为“新Xbox体验”彻底改革了旧界面，以增加用户个性化（例如3D头像）并增强这个控制台的社交方面。 同时，一个新的“杀手级应用”以物理配件的形式出现：Kinect。 没有人知道它的结果，但它证明是一个非常吸引人的话题。
...
这次攻击（_双关语_）要求用户提取NAND的内容（也就是制作一个转储），并使用一个自动化脚本，将转储中的控制台唯一信息与系统`2.0.1888`的通用镜像（操作系统的第一个修订版）合并。
...
结合所有这些信息，可以自动化一个试错猜测器，直到整个签名被暴力破解。 这就是时序攻击的实质。 一旦提取了秘密代码，它可以应用于修改后的`2.0.1888`镜像，一旦这个镜像被刷入游戏机的NAND，控制台就会认为它是有效的。 从那时起，用户可以更新到一个易受King Kong攻击的版本。
