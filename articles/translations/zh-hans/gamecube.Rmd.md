...
在SGI在图形市场失去主导地位后，任天堂需要寻求新的合作伙伴。

![Gekko的构成](cpu/cpu_features.png) {.open-float.no-borders}

IBM似乎是一个很有希望的候选者： 除了在大型计算机方面的杰出成就外，他们最近还与摩托罗拉和苹果公司结盟，开发出一种足以与英特尔在个人电脑市场的统治地位相抗衡的中央处理器。 由此产生的产品是一系列携带名称为**PowerPC**的处理器，它们被选中为99%的苹果Macintosh电脑和一些嵌入式系统提供*动力*。

很快，任天堂需要一款强大而廉价的处理器，为了符合这些要求，IBM选择了它过去的一个设计，即*PowerPC 750CXe*(曾用于iMac G3的后期型号，也称为*2001年初夏*)，并增强了它的功能，以满足游戏开发者的需求。 结果就是**PowerPC Gekko**处理器，主频为**486 MHz**。

{.close-float}

### 特点

让我们来看看Gekko为何如此与众不同，为此，我们需要首先了解750CXe提供的东西[@cpu-750cxe]：

- **PowerPC指令集架构（ISA）**：不详细深入介绍，它是*另一种*32位的精简指令集（RISC）。 750CXe采用了v1.10的规范。
- 外部**64位数据总线**：虽然ISA可以适应32位总线，但我们仍然需要在不影响性能的情况下传输更宽的数据块（在下一节中会解释）。
- **双发射超标量**： 如果有所需的单元，CPU可在流水线的同一阶段处理多达两条指令。 如果队列中包含分支指令，则可能的并发指令数会增加到**三条**。
- **乱序执行**：CPU可以重新排列指令的顺序，以使其所有单元都保持工作状态，从而提高效率和性能。
- **两个整数单元**： 与超标量和失序模型相结合，可增加单位时间内完成的整数运算次数。
- 拥有32位和64位寄存器的**集成浮点运算单元**：加速浮点数和双精度浮点数操作。
- **四级流水线(带有额外的优点)**A: [这里](game-boy-advance#cpu)是先前有关指令流程的简介。 在750CXe中，浮点运算被分为三个更多的阶段(总共7个阶段)，而装载存储操作被分为两个阶段(总共5个阶段)。
...
- 增强的指令集，包括**50条新的SIMD指令**：这些指令可以在一个周期内操作两个32位浮点数或一个64位浮点数。 因此，新的SIMD指令将加速向量计算，特别在几何变换过程中非常有用。
  - 不要与摩托罗拉的SIMD扩展（AltiVec）混淆，AltiVec用于高端的G4 Mac电脑。
...
> 在您的[Nintendo 64 文章](nintendo-64)上，您提到该系统有一颗64位的CPU，但GameCube的CPU是32位的。 Nintendo是否降级了他们的游戏机呢？
