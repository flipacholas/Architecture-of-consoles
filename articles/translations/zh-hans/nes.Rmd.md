...
- **Family Computer**（又称 _Famicom_）最初的版本，只在日本发售。 玩具般外观的设计带有两个不可拆卸的手柄 (第二个手柄内置麦克风) 、一个用于光枪 (被称为“_Zapper_”) 的前置接口以及RF视频输出 (使用 NTSC-J 制式) ，卡带插槽上还有用来扩展音频功能的额外引脚。
- **Nintendo Entertainment System**（又称 _NES_）为北美、欧洲和澳洲用户重新设计的版本，外观和机制类似于普通的 VHS/Betamax 播放器。 从技术上看，NES 的手柄可以插拔（没有麦克风），视频输出增加了 NTSC 或 PAL 制式的复合 RCA 接口，但是音频扩展引脚则被反盗版系统占用了。 作为补偿，外壳的底部安装有一个从未被使用的“扩展端口”，卡带可以从额外引脚与该端口通信 [@general-cartridge]。
...
例如，任天堂的《超级马力欧兄弟》（Super Mario Bros）使用了他们称之为_NES-NROM-256_的布局，该布局由32KB的程序ROM和8KB的图形用“Character RAM”组成（我们将在“图形”部分中看到更多关于它的信息）[@cpu-nrom]。 _NES-NROM-256_ 还提供了 3 KB 额外的 WRAM，尽管游戏没有使用它。
...
这一音频电路通常被称为**可编程声音发生器（Programmable Sound Generator）**（PSG），这大致意味着它只能产生预定义的一组波形，在这种情况下，这个说法_大部分_属实。 APU通过**五个音频通道**来排列音频数据——每个通道分别保留用于特定波形或信号。 每个通道包含不同的属性，这些属性会改变波形的音调、声音、音量和/或持续时间。 它们被连续混合并通过输出音频信号发送。
...
在幕后，噪声发生器依赖于信封发生器(类似于脉冲通道)，它通过OR门随机静音[@audio-apunoise]。 静音的条件取决于连接到反馈回路的 15 位移位寄存器的值。 总体来说，这使得电路输出具有_伪不可预测_模式的信号，因此产生噪音。
...
展望未来，使用频谱图进行声音分析在其他文章中将会派上用场，无论是用于简单分析还是与其他系统进行比较。 请注意，这些图表并不一定是_主要工具_，尤其是在声音样本被混合了太多信道/乐器时（极大地增加了其分解的难度）。 但我认为它们将为任何类型的客观研究提供一个坚实的起点。
