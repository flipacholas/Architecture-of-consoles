...
数据被打包成**128位**的数据包进行传输[@cpu-cell_arch]。 在数据包没有重叠的情况下，每个环可以承载**三组并发传输**。 EIB 以**命令信用（command credits）**模式工作，具体来说，每当一个组件需要发起传输时，它就会给 EIB 里用于管理环内流量的**数据仲裁器（Data Arbiter）**发送一个请求。 当这个请求被批准的时候，数据包就会进入环内，同时收到一个带有元数据，可以让仲裁者来管理传输的“令牌”。 另外，部分组件，比如用于访问主内存的** MIC**（Memory Interface Controller）会拥有比其他组件更高的优先级。 最后，仲裁者不会将路径长于半个环的数据包置入环内。
