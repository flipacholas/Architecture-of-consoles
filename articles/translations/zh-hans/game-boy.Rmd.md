...
如果您读过NES的文章，您或许会记得 PPU 是为 CRT 电子束设计的。 但很显然，GameBoy使用的是液晶屏幕。 不过，这颗新的 PPU 采用了同一种技术，因为液晶屏幕也是逐行刷新的。 如此一来，主机也有了 [如同CRT一般的特效](nes#tab-5-4-background-split)，让开发者能够尽情发挥自己的想象力。
...
这个功能听上去很傻： 因为窗口图层没有透明度，它完全遮盖住了背景。您可能会想，“都遮住了还看个毛啊？” 嗯...... 其实视窗图层可以与背景 **合用**，分别显示屏幕的各部分。 该图层主要是为了在屏幕底部显示固定信息，只不过在 NES 上需要执行 [复杂的定时写入](nes#tab-5-4-background-split)，但 GameBoy 的 PPU 可以自动处理。
