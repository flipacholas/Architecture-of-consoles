...
Nintendo 64的主处理器的起源是 **MIPS R4000**, 硅图（Silicon Graphic）的高级CPU 于1991年发布，R4000最明显的新颖之处是包含了 **64位能力**，这导致了总线、寄存器和计算单元的规模增加，以便有效地操纵64位数据。 另一方面，开发者通过新的 **MIPS III** 指令来访问这些能力。 总之，R4000使新的应用程序可以操纵更大的数据块，而不需要消耗额外的周期。

不管怎样，R4000是昂贵的产品(大约400美元 [@cpu-r4000demo])，这使得游戏主机不能使用它。 然而，任天堂不想放弃它最先进的产品， 这样他们就选择了一个来自NEC的叫做 **R4300i**的低端变体。

最后，任天堂选择的CPU变成了运行于 **93.75 MHz** [@cpu-anatomy] 的**NEC VR4300** 。 这个兼容硅图MIPS R4300i的CPU有以下特性[@cpu-nec]：
...
- 内部 **64 位总线** 连接到一个 **外部32 位数据总线**：尽管内部运行时，双词不会降低性能，CPU 仍然需要额外花费周期来从外部系统中处理64位的数据。 这是R4300i的一处阉割(R4000有完整的64位数据总线)。
...
系统实际包含 **4.5 MB的内存**，但它使用了 **9 位** 数据总线连接，这个数据总线中为GPU 保留了第9位 (更多详情在“图形”部分)。 因此，除GPU外，每个组件都只能寻址 ** 4 MB**。

![此系统的内存布局。 我假设CPU-RCP总线速度是RCP的时钟速度或CPU的时钟速度，但我还无法确认这一点。](memory.png)

安装在主板上的内存类型是 **Rambus DRAM** (RDRAM) [@cpu-memy], 这只是与 SDRAM竞争下一个标准的另一种设计。 RDRAM使用 **串行连接** (一次传输一位) ，而SDRAM 则使用 **并行连接** (一次传输多个位)。

RDRAM延迟与安装的内存芯片数量成正比 [@cpu-rdram]。 考虑到这个系统的内存容量，由此产生的延迟是很重要的(一篇beyond3d论坛的帖子认为延迟大约是 **640 纳秒** [@cpu-beyondrsp])。 虽然内存使用了 **250 MHz**的高频率来补偿这个问题 (比CPU快大约2.6倍)。 任天堂宣称RDRAM能够提供高达500MB每秒的连续读写速度。

此外，beyond3d论坛上的一些讨论声称任天堂选择在主板上使用NEC的uPD488170L内存芯片[@cpu-beyondrdar]。 这些芯片实现了一种叫做“Rambus Signaling Logic”的技术，这种技术使传输速率翻倍[@cpu-data]。 这可能解释了为什么一些信息源声明“有效”频率是500 MHz。

最后，这台主机上可用的 RAM大小 **可以通过安装 *扩展包* 来增加**。那是一个包含4.5 MB内存的外观奇特的小盒子。 奇怪的是，内存总线必须被终结， 所以控制台总是装配一个终端器 (称为 *跳线包*) 安装在扩展包的地方。 现在，您可能会问，如果您在没有安装任何 *包* 的情况下打开主机，会发生什么情况？ **实际上什么都没有**，你得到了一个空白屏幕！
...
- 一个**纹理单元**: 使用 4 KB 的专用内存(称为“TMEM”) 处理纹理，允许最多使用8个图块进行纹理处理。 它可以对它们执行以下操作：
  ...
  - **透视校正**: 将纹理映射到三角形的选用算法。 与<a href=“playstation#graphics”>其他逆映射算法</a>不同，该算法考虑了每个图元的深度值，从而获得更好的结果。
- 一个**彩色组合器**: 混合和插值多层颜色(例如应用阴影)。
- 一个**混合器**: 根据当前帧缓冲区混合像素以应用半透明、抗锯齿、雾、抖动。 它还执行Z缓冲(稍后会详细介绍)。
...
![塞尔达传说：时之笛(1998年)：<br>785个三角形.](link_ocarina_64){.toleft model3d="true"}

![星之卡比64 (2000).<br>516个三角形.](kirby_cristals_64){.toright model3d="true"}

### 现代可见表面判定

如果您已经阅读过上一个主机的文章， 你遇到了关于 [表面](sega-saturn#an-introduction-to-the-visibility-problem) 可见性的问题，并且现在可能认为多边形排序是解决这一问题的唯一方法。 嗯，在本系列中，RDP首次采用了一种基于硬件的方法，称为**Z缓冲**。 简而言之，RDP在内存中分配一个名为**“Z缓冲区”**的额外缓冲区。 它具有与帧缓冲区相同的尺寸，但不存储RGB值，而存储相对于相机的最近像素的深度（Z值）。
...
- 采样率可以达到44.1千赫，但使用最大采样率会占用大量CPU周期。
...
由于这些原因，玩家可能会注意到N64版游戏含有质量较低的音乐或重复乐谱。 尽管一个常见的解决方法是实现一个音乐音序器，该音序器在运行时使用预先填充的一组声音（类似于MIDI音乐）“构造”采样。
...
与以前的卡带系统不同， Nintendo 64 遵循复杂的引导过程，在实际游戏运行之前准备好所有硬件。 用户一打开主机就执行此操作，这与基于CD的同时代产品捆绑[BIOS](playstation#operating-system)或[IPL](sega-saturn#operating-system) [@operating_system-ipl] 非常相似。

这些例程也称为**初始程序加载**（IPL），工作方式如下[@operating_system-boot]：

1. 用户打开主机。
2. **PIF-NUS** (主板上的一个单独芯片) 将主CPU置于无限重置状态，直到PIF-NUS验证游戏卡带中的CIC芯片。
    - PIF-NUS和CIC芯片将分别在I/O与反盗版部分进一步介绍。
...
4. IPL2 初始化RDRAM和CPU缓存。 然后，它将游戏ROM的第一兆字节复制到RDRAM中。 这一部分包含下一个启动阶段，名为 **IPL3**。
5. IPL3启动操作系统（即虚拟内存和异常向量），设置程序状态（即堆栈指针），最后继续调用游戏的启动例程。

由于IPL3位于游戏卡带中，并非每个游戏都捆绑了相同的代码。 据推测，这些变体与在卡带中发现的不同CIC芯片有关。
...
官方SDK中的库含有若干抽象层，用于指挥RCP。 例如，C结构体（如<strong x-id=“1”>图形二进制接口</strong>或'GBI'）被设计为可以更容易地组合必要的显示列表，这同样适用于音频函数（其结构体被称为<strong x-id=“1”>音频二进制接口</strong>或'ABI'）。

在微代码开发方面，任天堂已经提供了一套微代码方案供选择。 然而，如果开发人员想要定制它，这确实是一项具有挑战性的任务：标量单元的指令集最初没有文档记录，但后来任天堂改变了立场，SGI最终发布了一些微代码编程文档。

用于开发的硬件包括SGI [@games-devkit] 提供的工作站，例如**Indy**机器，它带有一个额外的子板叫做**U64**其中包含零售主机的硬件和 I/O 。 他们还为Windows计算机提供了工具[@games-u64]。
...
![安装在主机上的64DD [@simply-amos]。](64dd/attached.png){.tabs-nested-last title="已安装好"}

磁性介质比卡带慢，传输速度高达1 MB/秒，但仍比4倍速CD-ROM光驱快。 磁盘是双面的，以“恒定角速度”运行（和后来的<a href=“gamecube#medium”>miniDVD</a>一样）。 最小的可读区域称为“块”，它是同心圆的一半。

这个驱动器中没有缓存内存，因此读取的位存储在RDRAM中以供执行。 任天堂将RAM扩展单元与64DD捆绑在一起，以弥补对更多内存的突然需求（在对所有64DD游戏的扩展内存进行标准化之外）。
...
尽管看起来很傻，任天堂还是打开了一扇门：<strong x-id=“1”>磁盘驱动器端口</strong>。
...
我想值得一提的是<strong x-id=“1”>博士 v64</strong>，该设备与磁盘驱动器形状相同，但包含CD-ROM驱动器。
...
当我还是个孩子的时候，我曾经用模拟器在奔腾II机器上玩过一些N64游戏，它并不是那么 *糟糕*，但在后来的几年里，我想知道*freck*是如何能够愉快地模拟复杂的64位机器的，因为除了其他问题以外，我的电脑几乎没有足够的内存来运行集成视频。
