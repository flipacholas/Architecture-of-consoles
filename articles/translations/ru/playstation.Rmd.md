...
Для начала, ЦП посылает геометрические данные (вершины) в ГП, заполнив его внутренний 64-байтовый FIFO-буфер **командами** (до трех штук). По сути, команда описывает, как и где рисовать один примитив.
...
GPU использует **примитивы в виде треугольников** для построения 3D-моделей. Это единственный возможный примитив, поэтому передние и задние фоны ничем не отличаются в плане композиции (оба состоят из треугольников). 2D игры следуют тому же принципу: это просто два треугольника, формирующие четырёхугольник (хотя ГП обеспечивает процесс автоматического построения спрайтов).

Растеризатор - это устройство, отвечающее за преобразование векторов в треугольники, а затем их в пиксели. Для этого он:

1. Собирает каждую вершину из трёх точек и вычисляет грани. Они формируют треугольник.
2. Анализирует область треугольника, чтобы понять, какие пиксели кадрового буфера он занимает. Любая секция треугольника, перекрывающая **точку сэмплирования**, превращается в пиксель.
...
Для применения световых эффектов над этими полигонами графический процессор предоставляет два алгоритма:
...
- **Затенение Гуро**: Каждая вершина примитива имеет свой собственный уровень освещённости. Затем, яркость между каждой точкой автоматически интерполируется.
  - Как можно себе представить, результаты выглядят более реалистично. С другой стороны, этот алгоритм не доступен для спрайтов.
...
Данный блок также включает в себя следующие возможные эффекты:
