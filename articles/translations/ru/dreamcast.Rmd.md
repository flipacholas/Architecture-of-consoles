...
Неудивительно, что Sega снова выбрала Hitachi для разработки своего CPU. Если вы читали [предыдущую статью о Sega Saturn](sega-saturn), то узрите! Я представляю вам следующее поколение процессоров SH: **SH-4**, работающее на космических **200 МГц** [@cpu-spec]. Итак, что в нём интересного?

- **5-ступенчатый конвейер**: До 5 инструкций могут находиться на отдельных ступенях конвейера одновременно (более подробную информацию можно найти в [предыдущей статье](sega-saturn#cpu)).
...
- **ARM7DI** работающий на ~2.82 МГц: Управляет Sound IC. Этот процессор программируется загрузкой небольшой программы (называемой [драйвером](super-nintendo#audio)) находящейся в SRAM, которая интерпретирует аудио данные и управляет Sound IC в соответствии с ними.
  - Кстати, похожий процессор можно найти в [Game Boy Advance](game-boy-advance).
...
![Sonic 3D Blast (1996) для Mega Drive.<br>Предшественник использует FM синтез для генерации звука на лету.](megadrive){.toleft video="true"}
...
BIOS так же содержит готовые функции для упрощения использования I/O операций в играх [@games-redream], как например чтение с GD-ROM диска.

### Оболочка

Если диск с игрой не вставлен, консоль загружает графическую оболочку.

![Оболочка, которая запускается если включить консоль без диска.](shell.png) {.open-float}
...
![Лого Windows CE на передней панели корпуса консоли.](windows_ce.jpeg) {.open-float}

На самом деле, предназначение этой 'ОС' было очень похоже на то, что Nintendo сделала с [Nintendo 64](nintendo-64#operating-system): предоставление программистам более богатого слоя абстракций для упрощения определенных операций.

Microsoft работала вместе с Sega чтобы портировать Windows CE на Dreamcast [@games-sdk]. Результатом была еще более урезанная версия Windows CE с минимальным набором компонентов, необходимых для работы с графикой, звуком и отладкой. Это включало в себя использование для разработки звездной IDE от Microsoft, **Visual Studio**.
...
Еще одним новшеством Dreamcast был **Visual Memory Unit** или 'VMU'. Он подключался к контроллеру и, кроме того что работал как карта памяти для сохранений, был полностью автономным устройством, включавшим в себя [@games-vmu]:
