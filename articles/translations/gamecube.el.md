...
{{< float_block >}}
  {{< linked_img src="cpu_features.png" alt="Construction of Gecko" >}} <figcaption class="caption">Construction of Gecko</figcaption>
{{< /float_block >}}
...
The history of this console's GPU has some interesting connections: Wei Yen, the director of N64's GPU ([}}#graphics">the RCP]({{< ref)), later founded Artx and landed a contract with Nintendo to develop their next-gen GPU: **Flipper**.

{{< float_group >}}

{{< float_block >}}
  {{< linked_img src="sunshine.png" alt="Super Mario Sunshine" >}} <figcaption class="caption">Super Mario Sunshine (2002)</figcaption>
{{< /float_block >}}
...
Flipper handles multiple services, so let's focus on the graphics component for now (since it's the one responsible for bringing our geometry to life). If you've been reading the [}}#graphics">N64 article]({{< ref), just letting you know that the core is now functional out of the box, so programmers won't need to worry about injecting code to make it work, nonetheless, there will some interesting parts that are customisable.

{{< centered_container >}}
  {{< linked_img src="flipper_pipeline.png" alt="Pipeline design of Flipper" >}} <figcaption class="caption">Pipeline design of Flipper</figcaption>
{{< /centered_container >}}
...
{{< float_block >}}
  {{< linked_img src="flipper_pipeline/database.jpg" alt="Database stage diagram" >}} <figcaption class="caption">Database stage diagram</figcaption>
{{< /float_block >}}
...
{{< float_block >}}
  {{< linked_img src="flipper_pipeline/vertex.jpg" alt="Vertex stage diagram" >}} <figcaption class="caption">Vertex stage diagram using indirect mode</figcaption>
{{< /float_block >}}
...
{{< float_block >}}
  {{< linked_img src="flipper_pipeline/texture.jpg" alt="Texture stage diagram" >}} <figcaption class="caption">Texture stage diagram using a default setup</figcaption>
{{< /float_block >}}
...
- **One Texture mapping unit** at the end of each Pixel unit (giving **four in total**): Together they process up to eight textures for our primitives (now mere pixels) at each cycle.
  ...
  - Finally, the unit also provides an **early [}}#modern-visible-surface-determination">z-buffer]({{< ref)**, **mipmapping** (processing a downsized texture instead, based on the level of detail) and **anisotropic filtering** (a welcoming improvement over the [}}#tab-1-2-reality-display-processor">previous filters]({{< ref) that provides greater detail with sloped textures).
...
{{< float_block >}}
  {{< linked_img src="flipper_pipeline/render.jpg" alt="Render stage diagram" >}} <figcaption class="caption">Render stage diagram</figcaption>
{{< /float_block >}}
...
- **Z-compare**: A late-stage [}}#modern-visible-surface-determination">Z-buffer]({{< ref). The engine will use 2 MB of embedded 1T-SRAM for Z-buffering calculations.
...
{{< float_block >}}
  {{< linked_img src="wind_waker.png" alt="Zelda Wind Waker" >}} <figcaption class="caption">The Legend of Zelda: The Wind Waker (2003)</figcaption>
{{< /float_block >}}
...
{{< float_block >}}
  {{< linked_img src="av_photo.jpg" alt="A/V photo" >}} <figcaption class="caption">A/V Connections on the back</figcaption>
{{< /float_block >}}

{{% inner_markdown %}}
- One named **Analog A/V** which is actually the good-old [}}#a-convenient-video-out">Multi Out]({{< ref). This is the most popular one.
...
{{< float_block >}}
  {{< tabs nested="true" >}}
    ...
    {{< /tab >}}
  {{< /tabs >}} <figcaption class="caption">Accessory slots at the bottom of the case</figcaption>
{{< /float_block >}}
...
{{< centered_container >}}
  {{< linked_img src="ipl/splash.png" alt="Gamecube Splash screen" >}} <figcaption class="caption">The official logo, shown after the boot animation finishes</figcaption>
{{< /centered_container >}}
...
{{< float_block >}}
  {{< tabs nested="true" >}}
    ...
    {{< /tab >}}
  {{< /tabs >}} <figcaption class="caption">Main Menu with multiple settings available</figcaption>
{{< /float_block >}}
...
{{< centered_container >}}
  {{< linked_img src="my_gamecube.jpeg" alt="My Gamecube" >}} <figcaption class="caption">My old Gamecube recently rescued from the attic <br>I bought it just cause I needed the controller for the Wii <br>(back then it was cheaper to buy the whole second hand lot!)</figcaption>
{{< /centered_container >}}
