...
models <- list(
  list(active = TRUE,
        ...
        title = "International"),
  list(caption = "The Famicom.\nReleased on 15/07/1983 in Japan",
        title = "Japanese")
  )
motherboard <- list(caption = "Showing the 'NES' variant")
set_frontmatter_yaml(long_title = "Architecture of the Nintendo Entertainment System (NES)",
                  short_title = "NES Architecture",
                  date = "2019-01-25", release_date = "1983-07-15",
                  name = "NES",
                  long_name = "Nintendo Entertainment System (NES)",
                  subtitle = "More than a 6502 machine",
                  generation = "3",
                  url_alias = "projects/consoles/nes",
                  models = models,
...
supporting_imagery(models = models, motherboard = motherboard)
```
...
```{r evolution, fig.cap=c("(ref:evolution)", caption2), fig.align='center', tab.nested=TRUE, centered=TRUE}
tab.name <- "Original"
image('nrom.png', "(ref:evolution)", tab.name = tab.name, tab.active = TRUE)
tab.name2 <- "Marked"
caption2 <- "The same PCB with important parts labelled.\n
The meaning of the 'Lockout' chip is explained in the 'Anti-piracy' section."
image('nrom_marked.png', caption2, tab.name = tab.name2)
```
...
```{r mappers, fig.cap=c(caption1, caption2), fig.align='center', tab.nested=TRUE, centered=TRUE}
caption1 <- "Simplified representation of how a mapper extends the addressing capabilities of the CPU.\n
...
image('mapper/mapper.png', caption1, class = "toleft", tab.name = "With mapper", tab.active = TRUE)
caption2 <- "The same setup but without a mapper installed.\n
While simpler and inexpensive, the CPU can only access a finite number of banks."
image('mapper/no_mapper.png', caption2, class = "toright", tab.name = "Without mapper")
```
...
```{r mariomappers, fig.cap=c("(ref:mariomappers)", caption2), fig.align='center', tab.nested=TRUE, centered=TRUE}
image('tsrom.png', "(ref:mariomappers)", tab.name = "Original", tab.active = TRUE)
...
image('tsrom_marked.png', caption2, tab.name = "Marked")
```
...
```{r memarch, fig.cap=caption, fig.align='center'}
caption <- "Memory architecture of the PPU"
image("ppu_content.png", caption, class = "centered-container")
```
...
```{r tiles, fig.cap=c(caption3), fig.subcap=c(caption1, caption2), fig.align="center", out.width = split_figure_width, tab.title="Tiles", tab.active = TRUE, tab.first=TRUE, tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel desktop-margined", fig.ncol = responsive_columns}
caption1 <- "Two pattern tables with multiple tiles squashed together."
image('ppu_mario/chr_map.png', caption1, tab.name = "All", tab.active = TRUE)
caption2 <- "A single tile."
image('ppu_mario/single.png', caption2, tab.name = "Single")
caption3 <- "Tiles Found in Character ROM (for demonstration purposes a default palette is being used)."
figcaption(caption3)
```
...
```{r background, tab.title="Background Layer", fig.cap=c(caption1, caption2, caption3), fig.align='center', tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel"}
caption1 <- 'Allocated background map.'
image('ppu_mario/nametable.png', caption1, tab.name = "Overall", tab.active = TRUE)
caption2 <- "Allocated background map with selected area marked."
image('ppu_mario/nametable_marked.png', caption2, tab.name = "Selected")
caption3 <- "Background map set up with vertical mirroring, which enables smooth horizontal scrolling. However, only one half can be used."
figcaption(caption3)
```
...
```{r sprite, tab.title="Sprite Layer", fig.cap=caption1, fig.align='center', tab.float=TRUE, tab.figure=TRUE, tab_class="pixel"}
caption1 <- 'Rendered sprite layer.'
image('ppu_mario/sprite_layer.png', caption1, class="pixel", float=TRUE)
```
...
```{r split, tab.title="Background split", fig.cap=caption1, fig.align='center', tab.float=TRUE, tab.figure=TRUE, tab_class="pixel"}
caption1 <- "Rendered background layer highlighting the two portions with different scrolling values defined. Only the second portion scrolls as Mario moves."
image('ppu_mario/split.png', caption1, class="pixel", float=TRUE)
```
...
```{r result, tab.title="Result", fig.cap=caption1, fig.align='center', tab.last=TRUE}
caption1 <- "Tada!"
image('ppu_mario/result.png', caption1, class="pixel", float=TRUE)
```
...
```{r diagonalscroll, fig.cap=c(caption1, caption2), fig.align='center', tab.title="Multi-Scrolling", tab.active = TRUE, tab.first=TRUE}
caption1 <- "Nametable setup for vertical scrolling (horizontal mirroring).\n
Super Mario Bros. 2."
image('secrets/multiscrolling_mirror.png', caption1, float=TRUE, class="pixel")
caption2 <- "Mario can run and fly, so the PPU needs to scroll diagonally. Notice the right edge showing the wrong colour palette! The left edge has a mask applied.\n
Super Mario Bros. 3"
image('secrets/multiscrolling.png', caption2, float=TRUE, class="pixel")
```
...
```{r tileswapping, fig.cap=c(caption1, caption2, caption3), fig.align='center', tab.title="Tile-Swapping", tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width=standard_figure_width}
caption1 <- "Hypothetical if it were rendered using tiles available during the first scan lines."
image('secrets/multiplexing_1.png', caption1, tab.name = "Scan 1", tab.active = TRUE)
caption2 <- "Hypothetical if it were rendered using tiles available during the later scan lines."
image('secrets/multiplexing_2.png', caption2, tab.name = "Scan 2")
caption3 <- "Actual frame displayed to the user."
image('secrets/multiplexing_complete.png', caption3, tab.name = "Displayed")
```
...
```{r fig.align='center', tab.title="Curious behaviour", tab.last=TRUE}
```
...
```{r pulse, fig.cap=c(caption1, caption2, caption3, caption4), fig.align='center', tab.title="Pulse", tab.active = TRUE, tab.first=TRUE, tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of the pulse 1 channel."
video('pulse_single_1', caption1, tab.name = "Pulse 1", tab.active = TRUE)
caption2 <- "Oscilloscope view of the pulse 2 channel."
video('pulse_single_2', caption2, tab.name = "Pulse 2")
caption3 <- "Oscilloscope view of all audio channels."
video('pulse_full', caption3, tab.name = "Complete")
caption4 <- "Mother (1989)."
figcaption(caption4)
```
...
```{r triangle, fig.cap=c(caption1, caption2, caption3), fig.align='center', tab.title="Triangle", tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of the triangle channel."
video('triangle_single', caption1, tab.name = "Triangle", tab.active = TRUE)
caption2 <- "Oscilloscope view of all audio channels."
video('triangle_full', caption2, tab.name = "Complete")
caption3 <- "Mother (1989)."
figcaption(caption3)
```
...
```{r noise, fig.cap=c(caption1, caption2, caption3), fig.align='center', tab.title="Noise", tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of the noise channel."
video('noise_single', caption1, tab.name = "Noise", tab.active = TRUE)
caption2 <- "Oscilloscope view of all audio channels."
video('noise_full', caption2, tab.name = "Complete")
caption3 <- "Mother (1989)."
figcaption(caption3)
```
...
```{r sample, fig.cap=c(caption1, caption2, caption3), fig.align='center', tab.title="Sample", tab.last=TRUE, tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of the sample channel."
video('sample_single', caption1, tab.name = "Noise", tab.active = TRUE)
caption2 <- "Oscilloscope view of the all audio channels."
video('sample_full', caption2, tab.name = "Complete")
caption3 <- "Mother (1989)."
figcaption(caption3)
```
...
```{r extrachannels, fig.cap=c(caption1, caption2), fig.align='center', tab.title="Extra Channels", tab.active = TRUE, tab.first=TRUE, tab.nested=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of Castlevania III (USA/Europe, 1989)"
video('castlevania_usa', caption1, tab.name = "American", tab.active = TRUE)
caption2 <- "Oscilloscope view of Akumajō Densetsu (Japanese version of Castlevania III, 1989)"
video('castlevania_jap', caption2, tab.name = "Japanese")
```
...
```{r tremolo, fig.cap=caption1, fig.align='center', tab.title="Tremolo", tab.last=TRUE, tab.float=TRUE, tab.figure=TRUE, tab_class="pixel", out.width = standard_figure_width}
caption1 <- "Oscilloscope view of Final Fantasy III (1990)"
video('tremolo_full', caption1)
```
...
```{r fdsside, fig.cap=c(caption1, caption2), fig.align='center', side_by_side=TRUE}
caption1 <- "The drive, where floppies are inserted (the photo shows a cardboard floppy inserted for protection). It either runs on six C batteries (1.5 V each) or a 3.6 W AC adapter."
image('fds/drive.png', caption1, class="toleft", no_borders=TRUE)
caption2 <- "The RAM adapter, fitted on the cartridge slot of the Famicom and connected to the drive though a cable."
image('fds/ram.png', caption1, class="toright", no_borders=TRUE)
caption3 <- "The two components that make up the Famicom Disk System (FDS)."
figcaption(caption3)
```
...
```{r fdsdrive, fig.cap=c(caption1, caption2, caption3), open_float_group=TRUE, fig.align='center'}
caption1 <- "FDS equipment mounted onto the Famicom."
image('fds/mounted.png', caption1, no_borders=TRUE, float=TRUE)
```
...
```{r fdsbiosside, fig.cap=c(caption1, caption2), fig.align='center', side_by_side=TRUE}
caption1 <- "Example of two retail games for the FDS. The blue 'flavour' of the disc was also dust proof."
image('fds/floppies.jpg', caption1, class="toleft")
caption2 <- "The FDS splash animation, waiting for the user to... uhm... insert a game."
video('fds_bios', caption1, class="toright")
```
