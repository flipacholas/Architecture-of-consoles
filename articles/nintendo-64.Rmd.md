...
(ref:memoryarchcaption) Memory layout of this system. I presume the CPU-RCP bus speed is either the RCP's clock speed or the CPU one, but I haven't been able to confirm that, yet.
...
This design is based on the philosophy that the GPU is not meant to be a 'simple' rasteriser like the [competitor's](`r ref("playstation#graphics")`). Instead, it should also be capable of **accelerating geometry calculations** (offloading the CPU), and for that, more circuitry will be needed.

### Architecture

This chip is divided into three main modules, two of them are used for graphics processing:

(ref:rspdiagramtitle) Reality Signal Processor

(ref:rspdiagramcaption) Architecture of the Reality Signal Processor (RSP).

```{r fig.cap="(ref:rspdiagramcaption)", fig.align='center', tab.title="(ref:rspdiagramtitle)", tab.first=TRUE, tab.active=TRUE}
image('RSP.png', "(ref:rspdiagramcaption)", float=TRUE)
```
...
This seems straightforward, but how does it perform these operations? Well, here's the interesting part: Unlike its competitors (PS1 and Saturn), **the geometry engine is not hard-wired**. Instead, the RSP contains some memory (4 KB for instructions and 4 KB for data) to store **microcode** `r cite("audio_video-ultra")`: A small program, with no more than 1000 instructions, that **implements the graphics pipeline**. In other words, it directs the Scalar Unit on how it should operate our graphics data. The microcode is fed by the CPU during runtime.

Nintendo provided different microcodes to choose from `r cite("audio_video-microcodes")` and, similarly to the [SNES' background modes](`r ref("super-nintendo#graphics")`), each one balances the resources differently.

(ref:rdpdiagramtitle) Reality Display Processor

(ref:rdpdiagramcaption) Architecture of the Reality Display Processor (RDP).

```{r fig.cap="(ref:rdpdiagramcaption)", fig.align='center', tab.title="(ref:rdpdiagramtitle)", tab.last=TRUE}
image('RDP.png', "(ref:rdpdiagramcaption)", float=TRUE)
```
...
- A **Texture Unit**: Processes textures using 4 KB of dedicated memory (called 'TMEM') allowing up to eight tiles to be used for texturing. It can perform the following operations on them:
  ...
  - **Perspective correction**: The chosen algorithm for mapping textures onto triangles. Unlike [others inverse-mapping algorithms](`r ref("playstation#graphics")`), this one takes into account the depth value of each primitive, achieving better results.
...
(ref:vpdiagramtitle) Vertex Processing

(ref:vpdiagramcaption) Primitive view of our scene. In order to save polygons, some characters are modelled using sprites (quads)

```{r fig.cap="(ref:vpdiagramcaption)", fig.align='center', tab.title="(ref:vpdiagramtitle)", tab.first=TRUE, tab.active=TRUE}
image('mario/wireframe.jpg', "(ref:vpdiagramcaption)", float=TRUE)
```
...
(ref:ppdiagramtitle) Pixel processing

(ref:ppdiagramcaption) Rendered frame (_Tada!_).

```{r fig.cap="(ref:ppdiagramcaption)", fig.align='center', tab.title="(ref:ppdiagramtitle)", tab.last=TRUE}
image('mario/result.png', "(ref:ppdiagramcaption)", float=TRUE)
```
...
Here are some examples of previous 2D characters for the [Super Nintendo](`r ref("super-nintendo")`) that have been redesigned for the new 3D era, they are interactive so I encourage you to check them out!

(ref:zeldamodelcaption) The Legend of Zelda: Ocarina of Time (1998).<br>785 triangles.

(ref:kirbymodelcaption) Kirby 64: The Crystal Shards (2000).<br>516 triangles.

```{r fig.cap=c("(ref:zeldamodelcaption)", "(ref:kirbymodelcaption)"), side_by_side=TRUE, fig.pos = "H"}
model_viewer('link_ocarina_64', class="toleft", "(ref:zeldamodelcaption)")
model_viewer('kirby_cristals_64', class="toright", "(ref:kirbymodelcaption)")
```

### Modern visible surface determination

If you've read about the previous consoles, you came across the never-ending problem regarding [visibility of surfaces](`r ref("sega-saturn#an-introduction-to-the-visibility-problem")`) and by now may think polygon sorting is the only way out of this. Well, for the first time in this series, the RDP features a hardware-based approach called **Z-buffering**. In a nutshell, the RDP allocates an extra buffer called **Z-Buffer** in memory. This has the same dimensions of a frame buffer, but instead of storing RGB values, each entry contains the depth (Z-value) of the nearest pixel with respect to the camera.
...
`r tab.simple("Pipeline Stalls", tab.first=TRUE, tab.active=TRUE)`
...
Nintendo carried on using the 'universal' [Multi Out](`r ref("super-nintendo.md#a-convenient-video-out")`) port found on its predecessor, bad news is that it **no longer carries the RGB signal!** It looks to me like another measure to save costs since RGB wasn't used anyway in the previous console.
...
(ref:audiodiagramcaption) Overview of how the audio pipeline is often programmed.

```{r fig.cap="(ref:audiodiagramcaption)", fig.align='center', centered=TRUE}
image("Audio.png", "(ref:audiodiagramcaption)", class = "centered-container")
```
...
(ref:observatorycaption) The Legend of Zelda: Majora's Mask (2000).<br>The music of this game is tied to its daunted atmosphere.

(ref:housecaption) Bomberman Hero (1998).<br>This game has nice and unique house-based soundtrack.

```{r fig.cap=c("(ref:observatorycaption)", "(ref:housecaption)"), side_by_side=TRUE, fig.pos = "H"}
video('observatory', class="toleft", "(ref:observatorycaption)")
video('redial', class="toright", "(ref:housecaption)")
```
...
Unlike previous cartridge-based systems, the Nintendo 64 follows a sophisticated boot process to prepare all of its hardware before the actual game runs. This is executed as soon as the user turns on the console and it's very similar to its CD-based contemporaries bundling a [BIOS](`r ref("playstation#operating-system")`) or [IPL](`r ref("sega-saturn#operating-system")`) `r cite("operating_system-ipl")`.

These routines are also referred to as **Initial Program Load** (IPL) and work as follows `r cite("operating_system-boot")`:
...
Hardware used for development included workstations supplied by SGI `r cite("games-devkit")`, like the **Indy** machine which came with an extra daughterboard called **U64** that contains the hardware and I/O of the retail console. Tools were supplied for Windows computers as well `r cite("games-u64")`.
...
Additionally, the PBUS branches out to another connector at the bottom of the N64's motherboard. This was meant to be used by the yet-unreleased **Nintendo 64 Disk Drive** (64DD), some sort of an 'extra floor' that contains a proprietary magnetic disk reader `r cite("games-dd")`. Its disks provide up to 64 MB of capacity. While only released in Japan, the Disk drive opened the door to an alternative (and cheaper) medium for distributing games.

(ref:ddtitle) Module

(ref:ddcaption) The Nintendo 64 Disk Drive `r cite("photography-amos")`.<br>Released on 01/12/1999 in Japan.

(ref:ddattachedtitle) Attached

(ref:ddattachedcaption) The 64DD attached to the console `r cite("photography-amos")`.

```{r fig.cap=c("(ref:ddcaption)", "(ref:ddattachedcaption)"), open_float_group=TRUE, tab.nested=TRUE, tab.float=TRUE}
image("64dd/module.png", "(ref:ddcaption)", tab.name="(ref:ddtitle)", tab.active=TRUE)
image("64dd/attached.png", "(ref:ddattachedcaption)", tab.name="(ref:ddattachedtitle)")
```

The magnetic medium is slower than cartridges, with transfer speeds of up to 1 MB/sec, but still faster than 4X CD-ROM readers. Disks are double-sided and operate at 'Constant Angular Velocity' (like the later [miniDVD](`r ref("gamecube#medium")`)). The smallest readable area is called 'block' and it's half of a concentric circle.
...
(ref:dv64title) Attached

(ref:dv64caption) The Doctor V64 attached to the console `r cite("photography-amos")`.

(ref:dv64attachedtitle) Back

(ref:dv64attachedcaption) The back of the V64 `r cite("photography-amos")`, showing some interesting A/V.

```{r fig.cap=c("(ref:dv64caption)", "(ref:dv64attachedcaption)"), open_float_group=TRUE, tab.nested=TRUE, tab.float=TRUE}
image("v64/attached.png", "(ref:dv64caption)", tab.name="(ref:dv64title)", tab.active=TRUE)
image("v64/back.png", "(ref:dv64attachedcaption)", tab.name="(ref:dv64attachedtitle)")
```
