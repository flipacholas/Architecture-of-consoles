...
At the heart of this console we find a powerful package called **Emotion Engine** or 'EE' designed by Sony and running at **~294.91 MHz** `r cite("cpu-rockin")`. This chipset contains multiple components, one of them being the main CPU. The rest are at the CPU disposal to speed up certain tasks.
...
- **MIPS III ISA**: A 64-bit RISC instruction set. *Wait, is it me or this is the same ISA found on a [competitor's console](`r ref("nintendo-64#cpu")`)?*. Not quite, Sony enhanced the ISA by adding some instructions from **MIPS IV** (prefetch and conditional move) along with their own SIMD extension called **multimedia instructions**. 
...
The core is complemented with a **dedicated floating point unit** (identified as 'COP1') that accelerates operations with 32-bit floating-point numbers (also known as `floats` in C). This is a peculiar block as it doesn't follow the IEEE 754 standard, most evident with its absence of `infinity` (computed as `0` instead) `r cite("cpu-krysto")`.

### A recognisable memory choice

Next to the Emotion Engine are two blocks of 16 MB of RAM, giving a total of **32 MB** of main memory. The type of memory used is **RDRAM** ([*déjà vu!*](`r ref("nintendo-64#memory-design")`)) which is accessed through a 16-bit bus.

(ref:memorydesigncaption) Memory design of the Emotion Engine. You can guess where the congestion will emerge.

```{r fig.cap="(ref:memorydesigncaption)", open_float_group=TRUE, fig.align='center'}
image('MemoryArch.png', "(ref:memorydesigncaption)", float=TRUE)
```
...
- Adding a 128-byte **Write Back Buffer**: Very similar to the [Write Gather Pipe](`r ref("gamecube#ibms-enhancements")`), but instead of waiting until it's 25% full, it will check the state of the bus (i.e congested or free) first.
...
Inside the same Emotion Engine package, there is yet-another processor called **Image Processing Unit** or 'IPU', this time designed for **image decompression**. As the successor of the [MDEC](`r ref("playstation#tab-2-3-motion-decoder")`), the IPU can be useful when a game needs to decode an MPEG2 movie without jamming the main CPU.
...
It's been two years since the rivals presented their [latest offering](`r ref("dreamcast")`). If you read the former article and just started reading this one, I presume you are *still* waiting for 'the thing' that makes the PS2 as powerful as it seemed back then. Now, let me introduce a *very* important set of components Sony fitted in the Emotion Engine, the **Vector Processing Units** or 'VPU'.
...
(ref:vp0title) Vector Processing Unit 0

(ref:vp0caption) Architecture of VPU0.

```{r fig.cap="(ref:vp0caption)", fig.align='center', tab.title="(ref:vp0title)", tab.first=TRUE, tab.active=TRUE}
image('VU0.png', "(ref:vp0caption)", float=TRUE, no_borders=TRUE)
```
...
(ref:vp1title) Vector Processing Unit 1

(ref:vp1caption) Architecture of VPU1.

```{r fig.cap="(ref:vp1caption)", fig.align='center', tab.title="(ref:vp1title)", tab.last=TRUE}
image('VUP1.png', "(ref:vp1caption)", float=TRUE, no_borders=TRUE)
```
...
Compared to using explicit data, procedural content is ideal for parallelised tasks, it frees up bandwidth, requires very little storage and it's dynamic (programmers can set parameters to achieve different results) `r cite("cpu-green")`. Many areas can highly benefit from this technique:
...
To sum up, procedural rendering is not a new technique, but thanks to the VPUs, it opens the doors to further optimisations and richer graphics. Nonetheless, is not a simple technique to implement and Sony R&D published various papers describing different approaches to use on their console `r cite("cpu-tumati")`.

### You define the workflow

With these new additions, programmers now have a lot of flexibility to design their graphics engines. To assist with this, Sony spent additional resources to devise and document efficient pipeline designs. The following are examples of graphics pipelines optimised for different types of workloads `r cite("cpu-stokes")`:
...
image("Serial.png", "(ref:diagramworkcaption2)", tab.name="(ref:diagramworktitle2)")
```
...
These have been so far examples from the theoretical point of view, but to explain a more 'practical' implementation, I'm going to refer to a video Jon Burton published regarding the development of one of their PS2 games `r cite("cpu-burton")`.

(ref:crashparticlescaption) Crash Bandicoot: The Wrath of Cortex (2001). Particles make the candle flame and the light coming from the window glass.

```{r fig.cap="(ref:crashparticlescaption)", open_float_group=TRUE, fig.align='center'}
image('Crash.jpg', "(ref:crashparticlescaption)", float=TRUE)
```
...
(ref:ffxcaption) Final Fantasy X (2001).

```{r fig.cap="(ref:ffxcaption)", open_float_group=TRUE, fig.align='center'}
image('ffx.jpg', "(ref:ffxcaption)", float=TRUE)
```
...
The GS has fewer features than other graphics systems [previously reviewed](`r ref("gamecube#graphics")`) on this site. Nonetheless, it's very fast at what it does.

`r close_float_group(with_markdown = TRUE)`

### Architecture and design

This GPU only does **rasterisation** and that is... Generating pixels, mapping textures, applying lighting and some other effects. This means there are no vertex transformations (these are covered by the VPUs). Also, this is a fixed-function pipeline, so no [fancy tweaking](`r ref("gamecube#creativity")`) or [shaders](`r ref("xbox#graphics")`) either, you are stuck with a fixed shading model (e.g. Gouraud).

(ref:gspipelinecaption) Pipeline design of the Graphics Synthesizer

```{r fig.cap=c("(ref:gspipelinecaption)"), fig.align='center', centered=TRUE}
image("GS_Pipeline.png", "(ref:gspipelinecaption)", class = "centered-container")
```

Looks pretty simple right? Well, let's dive deeper to see what happens at each stage.

(ref:gspreptitle) Pre-Processing

(ref:gsprepcaption) Pre-processing stage.

```{r fig.cap="(ref:gsprepcaption)", fig.align='center', tab.title="(ref:gspreptitle)", tab.first=TRUE, tab.active=TRUE}
image('gs_pipeline/Preprocessing.png', "(ref:gsprepcaption)", float=TRUE, no_borders=TRUE)
```
...
(ref:gsrastertitle) Rasterization

(ref:gsrastercaption) Rasterizing stage.

```{r fig.cap="(ref:gsrastercaption)", fig.align='center', tab.title="(ref:gsrastertitle)"}
image('gs_pipeline/Rasterizing.png', "(ref:gsrastercaption)", float=TRUE, no_borders=TRUE)
```
...
(ref:gstexttitle) Texturing

(ref:gstextcaption) Texture mapping stage

```{r fig.cap="(ref:gstextcaption)", fig.align='center', tab.title="(ref:gstexttitle)"}
image('gs_pipeline/Textures.png', "(ref:gstextcaption)", float=TRUE, no_borders=TRUE)
```
...
The pixel unit performs **perspective correction** to map textures onto the primitives (a great improvement considering the previous [affine mapping](`r ref("playstation#tab-4-5-textures")`) approach). Moreover, it also provides **bilinear and trilinear filtering**, the latter one is used alongside mipmapped textures.

(ref:gstesttitle) Testing

(ref:gstestcaption) Pixel Testing stage

```{r fig.cap="(ref:gstestcaption)", fig.align='center', tab.title="(ref:gstesttitle)"}
image('gs_pipeline/Tests.png', "(ref:gstestcaption)", float=TRUE, no_borders=TRUE)
```
...
- **Depth Test**: Compares the pixel's Z-value against the corresponding Z-value in the [Z-buffer](`r ref("nintendo-64#modern-visible-surface-determination")`). This avoids processing pixels that would be hidden behind other pixels.

(ref:gsposttitle) Post-Processing

(ref:gspostcaption) Post-Processing stage

```{r fig.cap="(ref:gspostcaption)", fig.align='center', tab.title="(ref:gsposttitle)", tab.last=TRUE}
image('gs_pipeline/Postprocessing.png', "(ref:gspostcaption)", float=TRUE, no_borders=TRUE)
```
...
It's worth mentioning that games like *Dragon Quest* implemented a custom lighting model called **Cel Shading** (a term I have mentioned [before](`r ref("gamecube#creativity")`)), however, in my previous articles I explained that the GPU was mainly responsible for this. In the PS2 case, the required colour calculations are presumably done by the Emotion Engine, since the GS isn't as flexible as other GPUs.
...
(ref:backavcaption) The right side of the console `r cite('photography-amos')`, as seen from the back. Showing A/C port, Digital audio port and AV Multi Out.

```{r fig.cap="(ref:backavcaption)", open_float_group=TRUE, fig.align='center'}
image('photos/ps2_back.png', "(ref:backavcaption)", float=TRUE)
```
...
The new audio chip is an incremental update of the old [SPU](`r ref("playstation#audio")`) called... **SPU2**! Improvements include the inclusion of **2 MB of internal memory** and **48 channels available** (two times the original amount).
...
(ref:noreverbcaption) Kingdom Hearts II (2005). Without reverb.

(ref:reverbcaption) Kingdom Hearts II (2005). With reverb.

```{r fig.cap=c("(ref:noreverbcaption)", "(ref:reverbcaption)"), side_by_side=TRUE, fig.pos = "H"}
video('goomy_noreverb', class="toleft", "(ref:noreverbcaption)")
video('goomy', class="toright", "(ref:reverbcaption)")
```
...
```{r fig.cap="(ref:frontcaption)", open_float_group=TRUE, fig.align='center'}
image('photos/ps2_front.png', "(ref:frontcaption)", float=TRUE)
```
...
(ref:iobaycaption) Back of PS2 showing hard drive bay (cover removed) `r cite('photography-amos')`.

(ref:iofronttitle) Front

(ref:iofrontcaption) Network adaptor as seen from the front `r cite('photography-amos')`. This particular model provided modem and ethernet ports.

(ref:iobacktitle) Back

(ref:iobackcaption) Network adaptor as seen from the back `r cite('photography-amos')`, with a hard drive fitted.

(ref:ioslimtitle) Slim

(ref:ioslimcaption) Back of the slim model showing a fixed ethernet port.

```{r fig.cap=c("(ref:iobaycaption)", "(ref:iofrontcaption)", "(ref:iobackcaption)", "(ref:ioslimcaption)"), fig.align='center', open_float_group=TRUE, tab.nested=TRUE, tab.float=TRUE}
image("photos/back_bay.png", "(ref:iobaycaption)", tab.name="(ref:iobaytitle)", tab.active=TRUE)
image("photos/harddrive_adaptor_front.png", "(ref:iofrontcaption)", tab.name="(ref:iofronttitle)")
image("photos/harddrive_adaptor_back.png", "(ref:iobackcaption)", tab.name="(ref:iobacktitle)")
image("photos/ps2_slim_back.jpg", "(ref:ioslimcaption)", tab.name="(ref:ioslimtitle)")
```
...
There's a **4 MB ROM** chip fitted on the motherboard that stores a great amount of code used to load a shell menu that the users can interact with, but it also provides system calls to simplify I/O access `r cite("cpu-rockin")`, which games rely on.

(ref:ioanimtitle) Boot animation

(ref:ioanimcaption) Splash animation after turning on the console.

(ref:iodisctitle) Valid disc

(ref:iodisccaption) PS2 logo showing after a valid PS2 game is inserted.

```{r fig.cap=c("(ref:ioanimcaption)", "(ref:iodisccaption)"), fig.align='center', tab.nested=TRUE, centered=TRUE}
image("bios/animated.jpg", "(ref:ioanimcaption)", tab.name="(ref:ioanimtitle)", tab.active=TRUE)
image("bios/game_splash.jpg", "(ref:iodisccaption)", tab.name="(ref:iodisctitle)")
```
...
(ref:biosmenutitle) Menu

(ref:biosmenucaption) Initial menu. Appears when there's no disc inserted.

(ref:biosbrowsertitle) Browser

(ref:biosbrowsercaption) Memory Card browser.

(ref:iosavestitle) Saves

(ref:iosavescaption) Saves browser. It shows up after selecting a memory card.

(ref:ioeditortitle) Editor

(ref:ioeditorcaption) Save editor, displayed after selecting a save.

(ref:ioopttitle) Options

(ref:iooptcaption) System Configuration.

```{r fig.cap=c("(ref:biosmenucaption)", "(ref:biosbrowsercaption)", "(ref:iosavescaption)", "(ref:ioeditorcaption)", "(ref:iooptcaption)"), fig.align='center', tab.nested=TRUE, centered=TRUE}
image("bios/menu.jpg", "(ref:biosmenucaption)", tab.name="(ref:biosmenutitle)", tab.active=TRUE)
image("bios/mem_list.jpg", "(ref:biosbrowsercaption)", tab.name="(ref:biosbrowsertitle)")
image("bios/save_list.jpg", "(ref:iosavescaption)", tab.name="(ref:iosavestitle)")
image("bios/save_editor.jpg", "(ref:ioeditorcaption)", tab.name="(ref:ioeditortitle)")
image("bios/options.jpg", "(ref:iooptcaption)", tab.name="(ref:ioopttitle)")
```
...
It is unprecedented the level of popularity this system achieved during the noughties, at the end of its lifespan (2013, after 13 years!) the game library was filled with 1850 titles `r cite("games-antista")`.

(ref:moskeetocaption) Mr Moskeeto (2001). Whenever someone argues about the abundance of PS2 games, I remember this one.

```{r fig.cap="(ref:moskeetocaption)", fig.align='center', open_float_group=TRUE}
image("mr_moskeeto.jpg", "(ref:moskeetocaption)", float=TRUE)
```
...
On the hardware side, Sony provided studios with dedicated hardware to run and debug games in-house. The initial devkits were bare boards stacked together to replicate the un-released hardware of the PS2. Later kits (named **Development Tool**), had a more presentable appearance, enhanced I/O and combined workstation hardware (running RedHat 5.2) with PS2 hardware to build and deploy the game in the same case `r cite("games-devkit")`.
...
(ref:khretailcaption) Kingdom Hearts II (2005). Typical retail game box and disc.

```{r fig.cap="(ref:khretailcaption)", fig.align='center', open_float_group=TRUE}
image("kh2_box.jpeg", "(ref:khretailcaption)", float=TRUE)
```

DVDs can hold up **4.7 GB** of data in the case of DVD-5 (the most common 'sub-format') or 8.5. GB in the case of DVD-9 (dual-layer version, less common) `r cite("games-dvd")`. There's actually a third format, DVD-10, which is double-sided but no games used it.
...
In terms of speed, CD-ROMs are read at 24x speed (so 3.6 MB/s) and DVD-ROMs are read at 4x speed (5.28 MB/s) `r cite("games-transfer")`.
...
(ref:rsodcaption) This error screen could appear if the drive is faulty... or a pirated copy was inserted.

```{r fig.cap="(ref:rsodcaption)", fig.align='center', open_float_group=TRUE}
image("bios/rsod.jpg", "(ref:rsodcaption)", float=TRUE)
```
...
`r tab.simple("Modchips", tab.active=TRUE, tab.first=TRUE)`
...
`r tab.simple("PS1 overflow", tab.last=TRUE)`

The PS2 stores a database file called `TITLE.DB` in MemoryCard which contains information used to optimise the emulation of PS1 games `r cite("anti_piracy-grand")`. When a PS1 game is inserted, the OS fetches the database file and loads the whole file in memory at a fixed address (*strike one*). The information parser is implemented using **`strncpy()`**, a function in C that copies strings (chain of characters) from one place to another.
...
After the slim revision was released, the exploit got patched (*I wonder how*). Curiously enough, it wasn't the last [blunder](`r ref("wii#the-fall-of-encryption")`) that exposed clumsy code.
